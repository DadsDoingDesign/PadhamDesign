...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!apiKey) throw new Error('Web3 API key is not configured');
      ...
      if (json.success) {
        setStatus({ submitted: true, submitting: false, info: { error: false, msg: contactContent.successMessage } });
        setFormData({ name: '', email: '', message: '' });
      } else {
        setStatus({ submitted: false, submitting: false, info: { error: true, msg: json.message || 'Something went wrong. Please try again.' } });
      }
    } catch {
      setStatus({ submitted: false, submitting: false, info: { error: true, msg: 'An error occurred. Please try again.' } });
    }
  };
    ...
    <section className="contact" id="contact">
      <div className="contact__container">
        <div className="contact__info">
          ...
          <div className="contact__details">
            <div className="contact__contact-item">
              <span className="contact__contact-label">Email</span>
              <a href={`mailto:${contactContent.email}`} className="contact__email">
                {contactContent.email}
              </a>
            </div>
            <div className="contact__contact-item">
              <span className="contact__contact-label">Phone</span>
              <a href={`tel:${contactContent.phone}`} className="contact__phone">
                {contactContent.phone}
              </a>
            </div>
            <div className="contact__contact-item">
              <span className="contact__contact-label">Studio</span>
        ...
        <form className="contact__form" onSubmit={handleSubmit}>
          ...
          {status.info.error && <p className="contact__error">{status.info.msg}</p>}
          {status.submitted && !status.info.error && <p className="contact__success">{status.info.msg}</p>}
